...
class CantusCerebraUno(nn.Module):
	def __init__(self, sorted_map, num_layers=6, in_dim=200, d_model=200, num_heads=8, dropout=0.1, convolution_set=[(1,), (3,), (5,)], d_ffn=800, out_dim=200):
		...
		self.TemporalEncoderLayers = nn.ModuleList([TransformerTemporalEncoderLayer(self.sorted_map, in_dim=in_dim, out_dim=out_dim, d_model=d_model, num_heads=num_heads, dropout=dropout, batch_first=True, convolution_set=convolution_set, d_ffn=d_ffn) for _ in range(self.num_layers + 1)])
